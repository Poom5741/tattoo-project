...
# ALL ISSUE STATES (for dependency checking)

Not all of these are ready to work — some are already closed. Use this list to
check whether a blocking dependency is resolved: if its `state` is `"CLOSED"`
it's done.

<all-issue-states>

!`gh issue list --state all --label {{ISSUE_LABEL}} --limit 200 --json number,title,state --jq '[.[] | {number, title, state}]'`
...
3. **Respect story order within an epic.** An issue is **unblocked** only if
   every story it depends on is already `done`. To check if a dependency is
   done, run `gh issue view <number> --json state` — if `state` is `"CLOSED"`
   the dependency is resolved. When an issue body says "Blocked by: #N",
   verify #N's actual GitHub state before treating it as blocked.
