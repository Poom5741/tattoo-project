...
tests/
  ...
  e2e/
    visual.spec.ts         — Playwright visual regression vs PNG baselines
  load/
    voucher.test.ts        — Documented spec for 50-concurrent POST idempotency
  baselines/
    *.png                  — PNG baselines generated by `pnpm baseline:capture`
    README.md              — How to add / update baselines
```
...
| PNG baselines in `tests/baselines/` | Run `pnpm baseline:capture` first |
| `npx playwright install chromium` | One-time browser download |
| D1 database seeded | `/api/designs` must return data |

If a baseline is missing the test is automatically skipped (`test.skip`).
